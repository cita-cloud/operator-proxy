...
func (g *Group) GoStartNode(f func(ctx context.Context, namespace, name string) error, ctx context.Context, namespace, name string) {
	g.wait.Add(1)

	go func(ctx context.Context, namespace, name string) {
		defer g.wait.Done()

		// Run and handle error
		if err := f(ctx, namespace, name); err != nil {
			g.locker.Lock()
			defer g.locker.Unlock()
			g.errors = append(g.errors, err)
		}
